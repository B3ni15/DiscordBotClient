...
export interface ThreadListProps {
  ...
  /**
   * "panel" is the standalone list with its own header. "inline" folds into the
   * channel sidebar as one more category and disappears when there is nothing
   * to show, so an empty channel does not carry a permanent empty box.
   */
...
export function ThreadList({
  ...
  // Inline, an empty list is simply not a section worth any space.
  if (inline && !error && !loading && visible.length === 0) return null;
  ...
  return (
    ...
    >
      {inline ? (
        <header className="flex shrink-0 items-center justify-between gap-2 px-2 pb-1">
          <h2 className="text-[11px] font-bold tracking-wide text-muted uppercase">Threads</h2>
          <button
            type="button"
        ...
        <header className="flex h-12 shrink-0 items-center justify-between gap-2 px-4 shadow-[0_1px_0_rgba(0,0,0,0.2)]">
          <h2 className="text-[15px] font-semibold text-bright">Threads</h2>
          <button
            type="button"
            onClick={() => setNonce((value) => value + 1)}
      ...
      <div className={`min-h-0 flex-1 overflow-y-auto ${inline ? "" : "px-2 py-2"}`}>
              ...
              return (
                <li key={thread.id} className="group flex items-center gap-1">
                  <button
                    ...
                    className={`flex min-w-0 flex-1 items-center gap-1.5 rounded px-2 py-1.5 text-left text-[15px] transition-colors ${
                      active
                  ...
                  <button
                    ...
                    className="shrink-0 rounded px-1.5 py-1 text-[11px] text-muted opacity-0 transition-opacity hover:text-bright focus-visible:opacity-100 group-hover:opacity-100 disabled:opacity-50"
                  >
